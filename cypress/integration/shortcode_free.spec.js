describe('Test Shortcode for free', function () {
  ...
  it('View the shortcode', function () {
    ...
    // click to view post
    cy.get('tr td a.row-title:contains("' + PREFIX + 'shortcode-single")')
      .first()
      .parent()
      .parent()
      .find('span.view a')
  ...
  it('View the shortcode (multiple feeds)', function () {
    cy.visit('/edit.php?post_type=post');
    // should have 1 post.
    cy.get('tr td a.row-title:contains("' + PREFIX + 'shortcode-multiple")').should(
      'have.length',
      1,
    );

    // click to view post
    cy.get('tr td a.row-title:contains("' + PREFIX + 'shortcode-multiple")')
      .first()
      .parent()
      .parent()
      .find('span.view a')
    ...
    cy.get('div.feedzy-rss div.rss_content').should(
      'have.length',
      Cypress.env('shortcode').multiple_results,
    );
    cy.get('div.feedzy-rss div.rss_content')
      .first()
      .should('contain', 'December 27, 2019 at 12:26 pm');
    cy.get('div.feedzy-rss div.rss_content')
      .last()
      .should('contain', 'December 17, 2019 at 8:36 am');
    ...
    // description
    cy.get('div.feedzy-rss div.rss_content p').should(
      'have.length',
      Cypress.env('shortcode').multiple_results,
    );
